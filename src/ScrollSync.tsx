...
export const ScrollSync: React.FC<PropsWithChildren<ScrollSyncProps>> = ({
  ...
  const syncScrollPositions = useCallback((scrolledPane: HTMLElement, groups: string[]) => {
  ...
  const registerPane = useCallback((node: HTMLElement, groups: string[]) => {
    groups.forEach((group) => {
      if (!panesRef.current[group]) {
  ...
  const unregisterPane = useCallback((node: HTMLElement, groups: string[]) => {
    groups.forEach((group) => {
      if (findPane(node, group)) {
        removeEvents(node);
